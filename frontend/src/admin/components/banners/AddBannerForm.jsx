...
const AddBannerForm = ({ showAddBannerForm, ToggleShowAddBannerForm }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const res = await AxiosHttp.post('/banner/create', data)
            if (!res.data.status) {
                toast.error(res.message)
                return
            }
            setLoading(false);
            toast.success("Add new banner successfully")
    ...
    return (
        <div className={`fixed inset-0 z-40  flex text-white transition-opacity duration-300 ${showAddBannerForm ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className={`relative overflow-y-auto no-scrollbar ml-auto h-full w-[400px] bg-[#0b0b0b] border-l border-gray-800 transform transition-transform duration-300 ease-in-out
                        ${showAddBannerForm ? 'translate-x-0' : 'translate-x-full'}`}
            >
                ...
                <div className='flex flex-col gap-2 px-4 '>
                    <div className={`transition-all duration-300 mt-16  `} >
                        ...
                        <label htmlFor="bannerAvatar" className="cursor-pointer w-1/2">
                    ...
                    <button type="submit" onClick={() => handleSubmit()} disabled={loading} className={`${loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}  bg-[#171717] text-white py-2 my-2 px-4 rounded-lg hover:bg-gray-800 transition duration-300`}>{loading ? 'Submiting...' : 'Add Banner'}</button>
                </div>
            </div>
        </div >
    )
